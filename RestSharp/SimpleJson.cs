 ...
 static class SimpleJson
    {
                ...
                else if (c == '\\')
                {
                    ...
                    else if (c == 'u')
                    {
                        ...
                        if (remainingLength >= 4)
                        {
                            ...
                            uint codePoint;
                            if (!(success = TryParse.UInt32(new string(json, index, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out codePoint)))
                                return "";
                            ...
                            if (0xD800 <= codePoint && codePoint <= 0xDBFF)  // if high surrogate
                            {
                                ...
                                {
                                    uint lowCodePoint;
                                    if (new string(json, index, 2) == "\\u" && TryParse.UInt32(new string(json, index + 2, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out lowCodePoint))
                                    {
        ...
        static object ParseNumber(char[] json, ref int index, ref bool success)
        {
            ...
            {
                double number;
                success = TryParse.Double(new string(json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture, out number);
                returnNumber = number;
            }
            else
            {
                long number;
                success = TryParse.Long(new string(json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture, out number);
                returnNumber = number;
            }
            index = lastIndex + 1;
            return returnNumber;
